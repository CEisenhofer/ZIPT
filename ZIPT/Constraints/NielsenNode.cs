...
public class NielsenNode {
    ...
    public NielsenNode? Backedge { get; set; }
    ...
    public Dictionary<NamedInt, (Interval<BigInteger> bound, DependencyTracker reason)> IntBounds { get; init; } = [];
    ...
    public NielsenNode(NielsenGraph graph, NielsenNode parent) : this(graph) {
        ...
        foreach (var e in parent.ConstraintsIntEq) {
            ConstraintsIntEq.Add(new IntEq(e.Poly, e.Reason));
        }
        foreach (var e in parent.ConstraintsIntLe) {
            ConstraintsIntLe.Add(new IntLe(e.Poly, e.Reason));
        }
        ...
        IntBounds = new Dictionary<NamedInt, (Interval<BigInteger> bound, DependencyTracker reason)>(parent.IntBounds);
    ...
    public void Apply(Subst subst, NielsenNode node) {
        ...
        foreach (var watched in watch) {
            ...
            if (bound.bound.HasLow)
                AddConstraint(IntLe.MkLe(Env.IntPDDManager.MkPDD((BigInteger)bound.bound.Min), n, bound.reason.Merge(subst.Reason)));
            else if (bound.bound.HasHigh)
                AddConstraint(IntLe.MkLe(Env.IntPDDManager.MkPDD(watched), n, bound.reason.Merge(subst.Reason)));
        }
    ...
    public SimplifyResult AddLowerIntBound(NamedInt v, InfNum<BigInteger> val, DependencyTracker reason) {
        ...
        if (!IntBounds.TryGetValue(v, out var bounds)) {
            ...
            IntBounds.Add(v, (i, reason));
            //AssertToZ3(i.ToZ3Constraint(v, Graph));
            return SimplifyResult.Restart;
        }
        if (val > bounds.bound.Max)
            return SimplifyResult.Conflict;
        if (val <= bounds.bound.Min)
            return SimplifyResult.Proceed;
        i = new Interval<BigInteger>(val, bounds.bound.Max);
        IntBounds[v] = (i, reason);
        //AssertToZ3(i.ToZ3Constraint(v, Graph));
        return SimplifyResult.Restart;
    }

    public SimplifyResult AddHigherIntBound(NamedInt v, InfNum<BigInteger> val, DependencyTracker reason) {
        ...
        if (val >= bounds.bound.Max)
            return SimplifyResult.Proceed;
        i = new Interval<BigInteger>(bounds.bound.Min, val);
        IntBounds[v] = (i, reason);
        //AssertToZ3(i.ToZ3Constraint(v, Graph));
        return SimplifyResult.Restart;
    }
    ...
    public bool ConsistentIntVal(IntVar var, InfNum<BigInteger> v) =>
        !IntBounds.TryGetValue(var, out var bounds) || bounds.bound.Contains(v);

    // lhs == rhs
    // No need to copy anything!
    public bool IsEq(PDD<BigInteger> lhs, PDD<BigInteger> rhs, out DependencyTracker? reason) {
        reason = null;
        IntEq eq = new(lhs, rhs, new DependencyTracker(0));
        switch (eq.Simplify(this)) {
            ...
            case SimplifyResult.Conflict:
                return false;
            default:
                if (!ConstraintsIntEq.TryGet(eq, out var res))
                    return false;
                reason = eq.Reason.Merge(res.Reason);
                return true;
        }
    }
    ...
    // No need to copy anything!
    public bool IsPowerElim(PDD<BigInteger> p, out DependencyTracker? reason) => IsZero(p, out reason) || IsNonPos(p, out reason);

    // p == 0
    // No need to copy anything!
    public bool IsZero(PDD<BigInteger> p, out DependencyTracker? reason) => IsEq(p, Env.ZeroInt, out reason);

    // p == 1
    // No need to copy anything!
    public bool IsOne(PDD<BigInteger> p, out DependencyTracker? reason) => IsEq(p, Env.OneInt, out reason);

    // lhs <= rhs
    // lhs - rhs <= 0
    // No need to copy anything!
    public bool IsLe(PDD<BigInteger> lhs, PDD<BigInteger> rhs, out DependencyTracker? reason) {
        reason = null;
        IntLe le = IntLe.MkLe(lhs, rhs, new DependencyTracker(0));
        switch (le.Simplify(this)) {
            case SimplifyResult.Satisfied:
                reason = le.Reason;
                return true;
            case SimplifyResult.Conflict:
                return false;
            default:
                if (!ConstraintsIntLe.TryGet(le, out var res))
                    return false;
                reason = le.Reason.Merge(res.Reason);
                return true;
        }
    }
    ...
    // lhs - rhs + 1 <= 0
    // No need to copy anything!
    public bool IsLt(PDD<BigInteger> lhs, PDD<BigInteger> rhs, out DependencyTracker? reason) {
        reason = null;
        IntLe le = IntLe.MkLt(lhs, rhs, new DependencyTracker(0)); 
        switch (le.Simplify(this)) {
            case SimplifyResult.Satisfied:
                reason = le.Reason;
                return true;
    ...
    // p < 0
    // No need to copy anything!
    public bool IsNeg(PDD<BigInteger> p, out DependencyTracker? reason) => IsLt(p, Env.ZeroInt, out reason);

    // p <= 0
    // No need to copy anything!
    public bool IsNonPos(PDD<BigInteger> p, out DependencyTracker? reason) => IsLe(p, Env.ZeroInt, out reason);

    // p > 0
    // No need to copy anything!
    public bool IsPos(PDD<BigInteger> p, out DependencyTracker? reason) => IsLt(Env.ZeroInt, p, out reason);

    // p >= 0
    // No need to copy anything!
    public bool IsNonNeg(PDD<BigInteger> p, out DependencyTracker? reason) => IsLe(Env.ZeroInt, p, out reason);
    ...
    public ModifierBase GetExtension(LocalInfo info) {
        ...
        foreach (var cnstr in ConstraintsStrEq) {
            ModifierBase? currentModifier = cnstr.Extend(info, []);
        ...
        foreach (var cnstr in ConstraintsStrMem) {
            ModifierBase? currentModifier = cnstr.Value.Extend(info, []);
            if (currentModifier is null)
                continue;
            if (bestModifier is null || currentModifier.CompareTo(bestModifier) < 0)
                bestModifier = currentModifier;
    ...
    public IEnumerable<NielsenEdge> ApplyExtension(LocalInfo info, ModifierBase modifier) {
        ...
        foreach (var child in modifier.Apply(info)) {
            int modCnt = info.ModCnt;
    ...
    public static BacktrackReasons SimplifyAndInit(LocalInfo info, NielsenEdge? edge) {
        ...
        while (true) {
            ...
            if (reason is not BacktrackReasons.Unevaluated and not BacktrackReasons.Extended) {
            ...
            if (outSideCnstr.Trivial && reason != BacktrackReasons.Extended) {
#if true
                // subsumption check
                // TODO: Optimize
            ...
            if (reason != BacktrackReasons.Extended) {
    ...
    public BacktrackReasons Simplify(LocalInfo info, NonTermSet modSet, DetModifier outSideCnstr, bool forceRewriteAll) {
        ...
        BacktrackReasons reason;
        while (restart) {
            restart = false;
            foreach (var c in AllConstraints) {
                if (c.Satisfied)
                    continue;
                reason = BacktrackReasons.Unevaluated;
                switch (c.SimplifyAndPropagate(info, modSet, outSideCnstr, ref reason)) {
                    ...
                    case SimplifyResult.Proceed:
                        break;
                    case SimplifyResult.Extended:
                        Debug.Assert(false);
                        return BacktrackReasons.Extended;
                    default:
        ...
        foreach (var eq in ConstraintsStrEq) {
            if (eq.Satisfied)
                continue;
            eq.SimplifyFinal(Env, outSideCnstr, forwardVarDep, largerVars, lowerBounds, true);
            eq.SimplifyFinal(Env, outSideCnstr, backwardVarDep, largerVars, lowerBounds, false);
        }
        Normalize();
    ...
    public NielsenNode MkChild(LocalInfo info,
        ...
        // FIRST remove (we might add elements with ids we removed)
        foreach (var c in toRemove) {
    ...
    public void AddConstraint(SymCharToken o, CharacterSet set) {
        ...
        var inter = value.IntersectWith(set);
        CharRanges[o] = inter;
        if (inter.IsEmpty) {
            IsGeneralConflict = true;
            CurrentReason = BacktrackReasons.CharacterRange;
    ...
    public static string ReasonToString(BacktrackReasons reason) => reason switch {
        ...
        BacktrackReasons.Extended => "Extended",
    ...
    public SolveResult Check(int dep, LocalInfo info) {
        ...
        try {
            ...
            if (!IsExtended && extensionEnum is null) {
                ...
                // Don't merge GetExtension and ApplyExtension, as computing the extension should happen eagerly and ApplyExtension lazily [Enumerator!]
                extensionEnum = ApplyExtension(info, GetExtension(info)).GetEnumerator();
            }

            // track changed regexes
            // do this AFTER computing which extension to take
    ...
    static string ToDfaDot(Str start, Dictionary<Str, (CharacterSet by, Str from)?> visited) {
        ...
        Dictionary<Str, uint> ids = [];

        uint GetId(Str s) {
            if (!ids.TryGetValue(s, out uint id)) {
                id = (uint)ids.Count;
                ids.Add(s, id);
            }
            return id;
        ...
        foreach (var v in visited) {
            if (!v.Value.HasValue)
                continue;
            uint src = GetId(v.Value.Value.from);
            uint tgt = GetId(v.Key);
            sb.Append('\t')
                .Append(src)
                .Append(" -> ")
                .Append(tgt)
                .Append(" [label=<")
                .Append(DotEscapeStr(v.Value.Value.by.ToString()))
                .AppendLine(">];");
        }
        foreach (var q in ids) {
            sb.Append('\t')
                .Append(q.Value)
                .Append(" [label=<")
                .Append(DotEscapeStr(q.Key.ToString()))
                .Append('>');
            if (q.Key.Equals(start))
