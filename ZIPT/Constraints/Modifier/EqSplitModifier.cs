...
public class EqSplitModifier : DirectedNielsenModifier {
    ...
    public override IEnumerable<NielsenEdge> Apply(LocalInfo info) {
        ...
        Str lhs1 = info.Env.StrManager.Extract(Eq.LHS, LhsIdx, Forwards);
        Str rhs1 = info.Env.StrManager.Extract(Eq.RHS, LhsIdx, !Forwards);
        Str lhs2 = info.Env.StrManager.Extract(Eq.LHS, LhsIdx, Forwards);
        Str rhs2 = info.Env.StrManager.Extract(Eq.RHS, LhsIdx, !Forwards);

        var padVar = info.Env.CreateFreshStrVar("o");
        if (Padding > 0) {
            lhs2 = info.Env.StrManager.Concat(lhs2, padVar, Forwards);
            rhs1 = info.Env.StrManager.Concat(padVar, rhs1, Forwards);
        }
        else if (Padding < 0) {
            lhs1 = info.Env.StrManager.Concat(lhs1, padVar, Forwards);
        ...
        if (!iEq2.Poly.IsZero)
            cnstr.Add(iEq2);
        info.CurrentNode.MkChild(info, [], [], cnstr, [Eq], true);
