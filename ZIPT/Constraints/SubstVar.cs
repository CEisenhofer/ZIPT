...
public class SubstVar : Subst {
    ...
    public IReadOnlyList<StrToken> Str { get; }

    public override bool IsEliminating => !Str.RecursiveIn(Var);

    public SubstVar(NamedStrToken v) {
        Var = v;
        Str = Array.Empty<StrToken>();
    }

    public SubstVar(NamedStrToken v, IReadOnlyList<StrToken> s) {
        Var = v;
        Str = s;
    }
    ...
    public override void CollectValueSymbols(NonTermSet nonTermSet) => Str.CollectSymbols(nonTermSet, []);
