...
public class MinTerms {
    ...
    public MinTerms Merge(MinTerms set, bool dropDiff = false) {
        ...
        while (i < Intervals.Count && j < set.Intervals.Count) {
            ...
            if (min1 < min2) {
                ...
                if (min2 > a.char2) {
                    ...
                    if (!dropDiff) {
                        if (!idMap.TryGetValue((Intervals[i].id, uint.MaxValue), out id))
                            idMap.Add((Intervals[i].id, uint.MaxValue), id = result.SetCount++);
                        result.Intervals.Add((min1, a.char2, id));
                    }
                    i++;
                    min1 = uint.MaxValue;
                    continue;
                ...
                if (!dropDiff) {
                    if (!idMap.TryGetValue((Intervals[i].id, uint.MaxValue), out id))
                        idMap.Add((Intervals[i].id, uint.MaxValue), id = result.SetCount++);
                    result.Intervals.Add((min1, min2 - 1, id));
                }
                ...
                if (min1 > b.char2) {
                    ...
                    if (!dropDiff) {
                        if (!idMap.TryGetValue((uint.MaxValue, set.Intervals[j].id), out id))
                            idMap.Add((uint.MaxValue, set.Intervals[j].id), id = result.SetCount++);
                        result.Intervals.Add((min2, b.char2, id));
                    }
                    j++;
                    min2 = uint.MaxValue;
                    continue;
                ...
                if (!dropDiff) {
                    if (!idMap.TryGetValue((uint.MaxValue, set.Intervals[j].id), out id))
                        idMap.Add((uint.MaxValue, set.Intervals[j].id), id = result.SetCount++);
                    result.Intervals.Add((min2, min1 - 1, id));
                }
                // proceed to "min1 == min2 case"
                // min2 = min1
            }
            if (!idMap.TryGetValue((Intervals[i].id, set.Intervals[j].id), out id))
                idMap.Add((Intervals[i].id, set.Intervals[j].id), id = result.SetCount++);
            if (Intervals[i].char2 < set.Intervals[j].char2) {
                // interval i ends first
                result.Intervals.Add((min1, Intervals[i].char2, id));
                ...
                result.Intervals.Add((min2, set.Intervals[j].char2, id));
        ...
        if (i < Intervals.Count) {
            ...
            if (!idMap.TryGetValue((Intervals[i].id, uint.MaxValue), out id))
                idMap.Add((Intervals[i].id, uint.MaxValue), id = result.SetCount++);
            result.Intervals.Add((min1 == uint.MaxValue ? a.char1 : min1, a.char2, id));
            while (++i < Intervals.Count) {
                a = Intervals[i];
                if (!idMap.TryGetValue((Intervals[i].id, uint.MaxValue), out id))
                    idMap.Add((Intervals[i].id, uint.MaxValue), id = result.SetCount++);
                result.Intervals.Add((a.char1, a.char2, id));
            }
        }
            ...
            if (!idMap.TryGetValue((uint.MaxValue, set.Intervals[j].id), out id))
                idMap.Add((uint.MaxValue, set.Intervals[j].id), id = result.SetCount++);
            result.Intervals.Add((min2 == uint.MaxValue ? b.char1 : min2, b.char2, id));
            while (++j < set.Intervals.Count) {
                b = set.Intervals[j];
                if (!idMap.TryGetValue((uint.MaxValue, set.Intervals[j].id), out id))
                    idMap.Add((uint.MaxValue, set.Intervals[j].id), id = result.SetCount++);
                result.Intervals.Add((b.char1, b.char2, id));
            }
        }
        return result;
    }
    ...
    public List<CharacterSet> ToCharacterSets() {
        ...
        foreach (var range in Intervals) {
            if (range.id >= rangesList.Count) {
                ...
                List<CharacterRange> ranges = [new CharacterRange(range.char1, range.char2 + 1)];
